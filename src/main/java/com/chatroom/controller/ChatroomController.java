...
public class ChatroomController extends CommonController {
    ...
    public void makePrivateChatroom(HttpServletRequest req , HttpServletResponse res) throws IOException {
        ...
        System.out.println("outmakeAlbum:"+targetId+"   isOwner:"+isOwner);
        if (isOwner == 1 && targetId != null) {
            System.out.println("makeAlbum:"+targetId);
            String name = req.getParameter("name");
            chserv.makePrivateChatroom(memberId,targetId,name);
            req.setAttribute("targetId",targetId);
            out.print(gson.toJson(new ResponseMessageDto(200,"聊天室建立成功")));
        }
    }
    public void makeGroupChatroom(HttpServletRequest req , HttpServletResponse res) {
        ...
        if (isOwner == 1) {
            for (String tid : targetIds) {
                if (tid != null && !"".equals(tid) && name != null && !"".equals(name)) {
