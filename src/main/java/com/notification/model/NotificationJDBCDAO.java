...
// 情境一 insert：新增一筆通知
	...
	public NotificationVO insert(NotificationVO notificationVO, Connection con) {
		final String INSERT_STMT = "INSERT INTO notification(member_id, context, status) values(?,?,0);";
		if (con != null) {
			try {
...
// 情境二 select：查詢某會員 id 所有通知
	...
	public List<NotificationVO> getAllById(Integer id, Connection con) {
		...
		final String SELECT_ONE_BYID = "select notification_id, member_id, context, time, status from notification where member_id = ?;";
